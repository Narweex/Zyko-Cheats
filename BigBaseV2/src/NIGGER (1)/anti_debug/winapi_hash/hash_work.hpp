...
NTSTATUS hash_RtlAdjustPrivilege(ULONG Privilege, BOOLEAN Enable, BOOLEAN CurrentThread, PBOOLEAN Enabled);
HDESK hash_CreateDesktopA(LPCSTR lpszDesktop, LPCSTR lpszDevice, DEVMODEA* pDevmode, DWORD dwFlags, ACCESS_MASK dwDesiredAccess, LPSECURITY_ATTRIBUTES lpsa);
BOOL hash_SwitchDesktop(HDESK hDesktop);
NTSTATUS hash_NtRaiseHardError(NTSTATUS error_status, ULONG number_of_parameters, ULONG unicode_string_parameter_mask, PULONG_PTR parameters, ULONG response_option, PULONG reponse);
//NTSTATUS hash_NtRaiseHardError(NTSTATUS ErrorStatus, ULONG NumberOfParameters, ULONG UnicodeStringParameterMask, PULONG_PTR *Parameters, ULONG ResponseOption, PULONG Response);
