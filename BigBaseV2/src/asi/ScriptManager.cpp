...
{
	return (GetModuleHandle(L"steam_api64.dll") == NULL) ? VER_1_0_2245_0_NOSTEAM : VER_1_0_2245_0_STEAM;
}
...
{
	int textureID = -1;
	if (big::g_pointers->m_file_register(&textureID, fileName.c_str(), true, fileName.c_str(), false))
	{
		LOG(G3LOG_DEBUG) << xorstr_("Registered File ") << fileName.c_str() << xorstr_(" with ID:") << textureID;
		return textureID;
	}

	LOG(FATAL) << xorstr_("Failed to register ") << fileName.c_str();
	return 0;
}
