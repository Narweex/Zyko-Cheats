...
namespace big::UserInterface
{
	...
	{
		DrawLeftText("0.0.5.Beta", m_PosX + 0.08f,
			m_DrawBaseY,
			0.24f,
			m_DescriptionFont,
	...
	void UIManager::DrawRect(float x, float y, float width, float height, rage::rgbaColor color)
	{
		GRAPHICS::DRAW_RECT(x, y, width, height, color.r, color.g, color.b, color.a, 0);
	}

	void UIManager::DrawSprite(const char* dict, const char* texture, float x, float y, float width, float height, rage::rgbaColor color, float rotation)
	{
	...
	void UIManager::DrawLeftText(const char* text, float x, float y, float size, Font font, rage::rgbaColor color, bool outline, bool shadow)
	{
	...
	void UIManager::DrawCenteredText(const char* text, float x, float y, float size, Font font, rage::rgbaColor color, bool outline, bool shadow)
	{
		HUD::SET_TEXT_CENTRE(true);
		DrawLeftText(text, x, y, size, font, color, outline, shadow);
	}

	void UIManager::DrawRightText(const char* text, float x, float y, float size, Font font, rage::rgbaColor color, bool outline, bool shadow)
	{
	...
	rage::vector2 UIManager::GetSpriteScale(float size)
	{
		int x;
		int y;
	...
	rage::vector2 UIManager::GetMousePos()
	{
		POINT point;
		GetCursorPos(&point);

		return rage::vector2{ static_cast<float>(point.x), static_cast<float>(point.y) };
	}
}
