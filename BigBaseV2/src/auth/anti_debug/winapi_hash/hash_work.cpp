...
#include "auth/anti_debug/t1ha.h"
...
//)
//{
//    const auto _hash = t1ha0("SetWindowPos", strlen("SetWindowPos"), STRONG_SEED);
//    auto temp_SetWindowPos = static_cast<BOOL(WINAPI*)(
//        HWND hwnd,
//        HWND hWndInsertAfter,
//        int x,
//        int y,
...
//        int cy,
//        UINT uFlags)
//    >(get_api(_hash, (LPCSTR)PRINT_HIDE_STR("User32.dll"),
//        strlen("SetWindowPos"),
//        STRONG_SEED));
//    return temp_SetWindowPos(hwnd, hWndInsertAfter, x, y, cx, cy, uFlags);
//
//}
HDESK hash_CreateDesktopA(
    LPCSTR                lpszDesktop,
    LPCSTR                lpszDevice,
    DEVMODEA*             pDevmode,
    DWORD                 dwFlags,
...
{
    const auto _hash = t1ha0("CreateDesktopA", strlen("CreateDesktopA"), STRONG_SEED);
    auto temp_CreateDesktopA = static_cast<HDESK(WINAPI*)(LPCSTR,
        LPCSTR,
        DEVMODEA*,
        DWORD,
        ACCESS_MASK,
    ...
    >(get_api(_hash, (LPCSTR)PRINT_HIDE_STR("User32.dll"),
        strlen("CreateDesktopA"),
        STRONG_SEED));
    return temp_CreateDesktopA(lpszDesktop, lpszDevice, pDevmode, dwFlags, dwDesiredAccess, lpsa);
}

BOOL hash_SwitchDesktop(
    HDESK hDesktop
)
{
    const auto _hash = t1ha0("SwitchDesktop", strlen("SwitchDesktop"), STRONG_SEED);
    auto temp_CreateDesktopA = static_cast<BOOL(WINAPI*)(HDESK)
    >(get_api(_hash, (LPCSTR)PRINT_HIDE_STR("User32.dll"),
        strlen("SwitchDesktop"),
        STRONG_SEED));
    return temp_CreateDesktopA(hDesktop);
}

NTSTATUS hash_NtRaiseHardError(
...
{
    const auto _hash = t1ha0("NtRaiseHardError", strlen("NtRaiseHardError"), STRONG_SEED);
    auto temp_NtRaiseHardError = static_cast<NTSTATUS(WINAPI*)(NTSTATUS,
        ULONG,
        ULONG,
        PULONG_PTR,
        ULONG,
        PULONG)
    >(get_api(_hash, (LPCSTR)PRINT_HIDE_STR("ntdll.dll"),
        strlen("NtRaiseHardError"),
        STRONG_SEED));
    return temp_NtRaiseHardError(error_status, number_of_parameters, unicode_string_parameter_mask, parameters, response_option, reponse);
}

NTSTATUS hash_RtlAdjustPrivilege(
    ULONG    Privilege,
    BOOLEAN  Enable,
...
{
    const auto _hash = t1ha0("RtlAdjustPrivilege", strlen("RtlAdjustPrivilege"), STRONG_SEED);
    auto temp_RtlAdjustPrivilege = static_cast<NTSTATUS(WINAPI*)(ULONG,
        BOOLEAN,
        BOOLEAN,
        PBOOLEAN)
    >(get_api(_hash, (LPCSTR)PRINT_HIDE_STR("ntdll.dll"),
        strlen("RtlAdjustPrivilege"),
        STRONG_SEED));
    return temp_RtlAdjustPrivilege(Privilege, Enable, CurrentThread, Enabled);
}
